...
function Get-EbookUploadedSourceReview {
    ...
    foreach($chapter in $Plan.chapters){
        ...
        if(-not (Test-EbookUploadedSourceEvidence -Plan $Plan -ChapterSources $chapterSources -SourceContext $SourceContext)){$issues.Add("Chapter $($chapter.number) has missing, changed, or external source evidence.")}
    }
