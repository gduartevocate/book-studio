...
. (Join-Path $PSScriptRoot 'EbookOutcomeAnalysis.ps1')
. (Join-Path $PSScriptRoot 'EbookStudyDiagrams.ps1')
...
function New-EngagementPlan {
    ...
    foreach ($chapter in $Plan.chapters) {
        ...
        [void]$items.Add([pscustomobject]@{
            ...
            altText = "$($visual.title): $(Get-StudyDiagramDescription -Model (Get-StudyDiagramModel -Title $visual.title))"
...
function Repair-EbookPackageOutputs {
    ...
    # Study diagrams are drawn by the generator, so a rebuild redraws the ones
    # nobody has edited; a book made before a drawing fix gets it here, and
    # generation (which ends here) keeps its own freshly drawn files.
    if ($engagementPlan) {
        $diagramRedraw = Update-EbookStudyDiagrams -OutputFolder $resolvedOutputFolder -EngagementPlan $engagementPlan -BrandProfile $brandProfile
        if ($diagramRedraw.refreshed.Count) {
            Write-EbookGeneratorProgress -Phase "Redrawing study diagrams" -Detail "Redrew $($diagramRedraw.refreshed.Count) study diagram(s) with the current layout: $($diagramRedraw.refreshed -join ", "). The previous files are in $($diagramRedraw.backupFolder)."
        }
        foreach ($keptDiagram in $diagramRedraw.kept) {
            $keptDetail = if ($keptDiagram.issues.Count) { " " + ($keptDiagram.issues -join " ") } else { "" }
            Write-EbookGeneratorProgress -Phase "Keeping an edited study diagram" -Detail "$($keptDiagram.file) was changed after it was generated, so it was kept as it is.$keptDetail"
        }
    }
